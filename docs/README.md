# Fabkit Documentation

**Fabkit** is a Svelte 5 UI component library for building desktop-style web applications.

---

## Contents

### Core

- [Getting Started](./getting-started.md) — installation, setup, SSR, dark mode
- [Theming](./theming.md) — `initTheme`, CSS variables, fine-tuning, dark mode

### Foundation

- [Skeleton](./components/skeleton.md) — base layout primitive (all visual props)

### Layout

- [Window](./components/window.md) — app shell
- [TitleBar](./components/title-bar.md) — window title bar
- [SideLayout](./components/side-layout.md) — sidebar + content
- [Leaflet](./components/leaflet.md) — adaptive two-pane stack
- [Paned](./components/paned.md) — resizable two-panel split
- [ToolbarView](./components/toolbar-view.md) — top/bottom bars + content
- [HBox](./components/hbox.md) — horizontal flexbox row
- [VBox](./components/vbox.md) — vertical flexbox column
- [Grid](./components/grid.md) — CSS Grid wrapper
- [Card](./components/card.md) — bordered container
- [Wrapper](./components/wrapper.md) — generic container
- [Clamp](./components/clamp.md) — constrain content size

### Navigation & Tabs

- [MenuSwitcher](./components/menu-switcher.md) — sidebar nav linked to StaticNotebook
- [StaticNotebook](./components/static-notebook.md) — page switcher
- [Notebook](./components/notebook.md) — dynamic tabbed content
- [TabsSwitcher](./components/tabs-switcher.md) — tab bar for Notebook
- [Tab](./components/tab.md) — single tab for TabsSwitcher
- [Accordion](./components/accordion.md) — expandable/collapsible panels
- [AccordionItem](./components/accordion-item.md) — single panel for Accordion

### Interactive

- [Button](./components/button.md) — action button
- [Switcher](./components/switcher.md) — toggle switch
- [SwitchRow](./components/switch-row.md) — row with label and switch
- [Slider](./components/slider.md) — range input
- [SpinRow](./components/spin-row.md) — row with label and spinner
- [Carousel](./components/carousel.md) — slide viewer
- [Dialog](./components/dialog.md) — modal confirmation
- [PopOver](./components/pop-over.md) — anchored floating panel
- [PopOverWrapper](./components/pop-over-wrapper.md) — popover positioning utility
- [ContextArea](./components/context-area.md) — right-click context menu
- [SlideOver](./components/slide-over.md) — bottom sheet / modal drawer
- [SlideOverContent](./components/slide-over-content.md) — content for SlideOver
- [Menu](./components/menu.md) — vertical item list
- [MenuItem](./components/menu-item.md) — single menu item
- [ActionRow](./components/action-row.md) — row for actions/settings
- [PreferencesGroup](./components/preferences-group.md) — group for settings
- [PreferencesPage](./components/preferences-page.md) — settings page layout
- [Tooltip](./components/tooltip.md) — floating label on hover/focus
- [Separator](./components/separator.md) — divider line

### Form Fields

- [TextField](./components/text-field.md) — text input
- [TextArea](./components/text-area.md) — multiline text input
- [SelectField](./components/select-field.md) — dropdown select
- [BaseField](./components/base-field.md) — field primitive (for custom fields)
- [Checkbox](./components/checkbox.md) — checkbox input
- [CheckboxGroup](./components/checkbox-group.md) — group of checkboxes
- [RadioGroup](./components/radio-group.md) — group of radio buttons
- [NumberField](./components/number-field.md) — numeric input
- [TagsInput](./components/tags-input.md) — multi-value tag input
- [Chip](./components/chip.md) — tag/badge
- [FileChooser](./components/file-chooser.md) — file input
- [ColorPicker](./components/color-picker.md) — color selection input

### Content

- [Image](./components/image.md) — versatile image component
- [TextRich](./components/text-rich.md) — styled prose container
- [Text](./components/text.md) — inline text helper
- [Kbd](./components/kbd.md) — keyboard shortcut display
- [LevelBar](./components/level-bar.md) — progress/level bar

### Data Display

- [DataTable](./components/data-table.md) — tabular data
- [EntryRow](./components/entry-row.md) — label/value row
- [SpinRow](./components/spin-row.md) — label + spinner row

### Miscellaneous

- [EngineDecorator](./components/engine-decorator.md) — engine context provider
- [Wrapper](./components/wrapper.md) — generic content wrapper

### Icons

- [Icons](./icons.md) — 1514 Phosphor icons
